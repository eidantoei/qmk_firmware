...
void td_alt_fn_finished(qk_tap_dance_state_t *state, void *user_data)
{
  if (state->count == 1) {
...
void td_alt_fn_reset(qk_tap_dance_state_t *state, void *user_data)
{
  if (state->count == 1) {
...
qk_tap_dance_action_t tap_dance_actions[] = {
  [TD_ALT_FN] = ACTION_TAP_DANCE_FN_ADVANCED(NULL, td_alt_fn_finished, td_alt_fn_reset),
};
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_BASE] = LAYOUT(
    ...
    TD(TD_ALT_FN),  CMD_EISU, KC_NUMS, KC_SYMS, CMD_KANA, KC_RALT
  ),
  [_NUMS] = LAYOUT(
  ...
  [_FN] = LAYOUT(
    ...
    RESET, _______, _______, _______, _______, _______, _______, _______, KC_HOME, KC_PGUP, _______, _______, _______,
